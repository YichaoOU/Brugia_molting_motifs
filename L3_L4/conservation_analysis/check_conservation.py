...
B_fimo = "/users/PHS0293/ohu0404/project/Brugia/FC4/L3_L4/L3_L4/L3_L4_Motif_Scanning/L3_L4_fore_Fimo/fimo.txt"
C_fimo = "elegans.fimo"
O_fimo = "OVOC.fimo"
	...
	for gene in B_fimo_pos[motif]:
		if gene in gene_list:
			...
			if C_fimo_pos[motif].has_key(ortholog1):
				...
				motif_cons[motif]["amg"].append("Brugia " + gene +" : " + ",".join(map(lambda x:str(x),B_adj_pos_array)) + "; C.elegans: " + ",".join(map(lambda x:str(x),C_adj_pos_array))+ "@" + ",".join(map(lambda x:str(x),C_pos_array)))
			if O_fimo_pos[motif].has_key(ortholog2):
				...
				motif_cons[motif]["amg"].append("Brugia " + gene +" : " + ",".join(map(lambda x:str(x),B_adj_pos_array)) + "; OVOC: " + ",".join(map(lambda x:str(x),O_adj_pos_array)) + "@" + ",".join(map(lambda x:str(x),O_pos_array)))
...
out = open("L3_L4_motif_conservation_analysis.tsv","wb")				
